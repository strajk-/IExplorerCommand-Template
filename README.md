## Overview
This repo contains a the bare minimum required for creating your own Windows 11 context menu extension with IExplorerCommand without having to install any dependencies on target machines you intend to deploy this.
...
- CustomShellManager Application handles the installation and uninstall of both the Context Menu Extension and the Root CA on the target machine, it also creates registry entries in HKEY_CLASSES_ROOT in case you defined file extensions in your AppxManifest.xml that aren't yet present in that registry path.
...
* [.NET Framework 4.8](https://dotnet.microsoft.com/en-us/download/dotnet-framework/net48), which is integrated into every Windows installation since Windows 10 v20H2, older Versions require it to be installed first.
...
- Minimal amount of files as resources, only shipping what is needed for the context menu, up to you if you want to add more, head over to the AppxManifest.xml and adjust at your own leasure as you add more resources into the CustomShellPackage folder.
...
- Easy to apply your own branding, just use a tool such as AstroGrep, search for files containing "CustomShell" as a name or content, rename and replace all instances it finds to what you want and you're good to go.
...
Console output should tell you that it successfully installed both the certificate attached to the executable and the context menu extension
